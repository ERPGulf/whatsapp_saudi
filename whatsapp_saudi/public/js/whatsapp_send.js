function generateAndSendPDF(frm, title, method) {
    const dialog = new frappe.ui.Dialog({
        ...
        primary_action: function () {
            ...
            frappe.call({
                method: method, // Dynamically pass the method name
                args: {
...
frappe.ui.form.on("Sales Invoice", {
    refresh: function (frm) {
        frm.add_custom_button(__('Send Pdf through WhatsApp'), function () {
            generateAndSendPDF(frm, 'Send PDF-A3 via WhatsApp', 'whatsapp_saudi.overrides.whtatsapp_notification.send_whatsapp_with_pdf1');
        });
    }
});
